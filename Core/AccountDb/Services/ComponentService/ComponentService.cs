    ...
    public class ComponentService : IComponentService
    {
        ...
        protected void CalculateComponentEventsStatus(Guid accountId, Guid eventsStatusId)
        {
            ...
            using (var data = AllCaches.StatusDatas.Write(request))
            {
                ...
                if (nextEvent != null)
                {
                    var noSignalImportance = Storage.EventImportance.Unknown;
        ...
        protected IComponentCacheReadObject RecalculateComponentStatuses(IComponentCacheReadObject component)
        {
            ...
            foreach (var unitTestRef in component.UnitTests.GetAll())
            {
                ...
                unitTestService.GetUnitTestResult(component.AccountId, unitTestRef.Id);
            }
            ...
            foreach (var metricRef in component.Metrics.GetAll())
            {
                ...
                metricService.GetActualMetric(component.AccountId, component.Id, metricTypeId);
            }
            ...
            if (component.CanProcess)
            {
                ...
                statusService.CalculateByChilds(accountId, component.UnitTestsStatusId, allUnitTestsStatusDataIds.ToArray());

                // обновим колбасу метрик явно (вдруг нет ни одной метрики)
                statusService.CalculateByChilds(accountId, component.MetricsStatusId, allMetricsStatusDataIds.ToArray());
                ...
                statusService.CalculateByChilds(accountId, component.ChildComponentsStatusId, childComponentsStatusDataIds.ToArray());
        ...
        public ComponentForRead GetComponentByIdNoCache(Guid accountId, Guid id)
        {
            return _storage.Components.GetOneById(id);
        }

        public ComponentForRead GetComponentBySystemName(Guid accountId, Guid parentId, string systemName)
        {
        ...
        public ComponentForRead CreateComponent(Guid accountId, CreateComponentRequestData data)
        {
            if (data == null)
            {
        ...
        private void Add(Guid accountId, Guid componentId, Guid parentId, string displayName, string systemName, Guid componentTypeId)
        {
            var parent = _storage.Components.GetOneById(parentId);
            ...
            Add(component, accountId);
        }

        private Guid CreateStatusData(ComponentForAdd component, EventCategory eventCategory, Guid accountId)
        {
        ...
        protected void Add(ComponentForAdd componentForAdd, Guid accountId)
        {
            ...
            if (componentForAdd.ParentId != null)
            {
                ...
                {
                    throw new ResponseCodeException(
                        Zidium.Api.ResponseCode.UnknownComponentTypeId,
            ...
            componentForAdd.ExternalStatusId = CreateStatusData(componentForAdd, EventCategory.ComponentExternalStatus, accountId);
            componentForAdd.InternalStatusId = CreateStatusData(componentForAdd, EventCategory.ComponentInternalStatus, accountId);
            componentForAdd.EventsStatusId = CreateStatusData(componentForAdd, EventCategory.ComponentEventsStatus, accountId);
            componentForAdd.UnitTestsStatusId = CreateStatusData(componentForAdd, EventCategory.ComponentUnitTestsStatus, accountId);
            componentForAdd.MetricsStatusId = CreateStatusData(componentForAdd, EventCategory.ComponentMetricsStatus, accountId);
            componentForAdd.ChildComponentsStatusId = CreateStatusData(componentForAdd, EventCategory.ComponentChildsStatus, accountId);
        ...
        public ComponentForRead GetOrCreateComponentInternal(Guid accountId, GetOrCreateComponentRequestData data, bool createNew)
        {
            if (data == null)
            {
                throw new ParameterRequiredException("Request.Data");
            ...
            lock (lockObj)
            {
                ...
                if (component == null)
                {
                    ...
                    if (component == null)
                    {
                        ...
                        var limitCheckResult = checker.CheckMaxComponents(_storage);
        ...
        public ComponentForRead GetOrCreateComponent(Guid accountId, GetOrCreateComponentRequestData data)
        {
            if (data == null)
            {
                throw new ParameterRequiredException("Request.Data");
        ...
        private void UpdateComponentProperies(Guid componentId, List<Api.ExtentionPropertyDto> properties)
        {
            var componentProperties = _storage.ComponentProperties.GetByComponentId(componentId);

            // удаляем
            var forDelete = componentProperties.Where(x => properties.All(y => y.Name.ToLower() != x.Name.ToLower())).ToArray();
            ...
            {
                var componentProperty = componentProperties.FirstOrDefault(t => t.Name.ToLower() == property.Name.ToLower());
        ...
        public void UpdateComponent(Guid accountId, UpdateComponentRequestData data)
        {
