...
namespace Zidium.Core.AccountsDb
{
    public class DefectService : IDefectService
    {
        ...
        public DefectForRead GetOrCreateDefectForEventType(Guid accountId,
            ...
            lock (typeof(DefectService))
            {
                using (var transaction = _storage.BeginTransaction())
                {
                    ...
                    if (defectId == null)
                    {
                        ...
                        defectId = CreateDefectInternal(accountId, title, createUser, comment, responsibleUser, null);
        ...
        public Guid ChangeStatus(Guid accountId,
        ...
        public Guid AutoReopen(Guid accountId, DefectForRead defect)
        {
            var userService = new UserService(_storage);
            var user = userService.GetAccountAdmin();
            return ChangeStatus(accountId, defect, DefectStatus.Reopened, user, "Автоматически переоткрыт из-за новой ошибки");
        }
        ...
        public Guid CreateAndCloseDefectForEventType(Guid accountId, EventTypeForRead eventType, UserForRead user)
        {
        ...
        private Guid CreateDefectInternal(Guid accountId, string title, UserForRead createUser, string statusComment, UserForRead responsibleUser, string notes)
        {
        ...
        public Guid CreateDefect(Guid accountId, string title, UserForRead createUser, UserForRead responsibleUser,
            string notes)
        {
