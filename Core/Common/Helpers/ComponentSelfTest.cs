...
namespace Zidium.Core.Common.Helpers
{
    ...
    public class ComponentSelfTest
    {
        ...
        public ComponentSelfTestResult Validate()
        {
            ...
            {
                result.Success = false;
                log.AppendLine("Нет ни одной проверки");
                unitTestControl.SendResult(UnitTestResult.Alarm, TimeSpan.FromMinutes(5), "Нет ни одной проверки");
        ...
        {
            _timer = new Timer((_) => Validate(), null, TimeSpan.Zero, period);
        }
