...
namespace Zidium.Core.Caching
{
    public class MetricTypeCacheStorage : AccountDbCacheStorageBase<MetricTypeCacheResponse, IMetricTypeCacheReadObject, MetricTypeCacheWriteObject>
    {
        ...
        public Guid CreateMetricType(string name, IStorage storage)
        {
            if (name == null)
            {
            ...
            lock (UpdateMapLock)
            {
                ...
                return GetOrCreateTypeId(name, storage);
            }
        }

        public Guid GetOrCreateTypeId(string name, IStorage storage)
        {
            if (name == null)
            {
                throw new ArgumentNullException("name");
        ...
        protected void RenameMap(Guid metricTypeId, string oldName, string newName)
        {
        ...
        protected void DeleteFromMap(Guid metricTypeId, string name)
        {
            var key = GetMapKey(name);
            lock (UpdateMapLock)
            {
        ...
        protected override MetricTypeCacheWriteObject LoadObject(AccountCacheRequest request, IStorage storage)
        {
            ...
            return MetricTypeCacheWriteObject.Create(metricType);
        }
    }
}
