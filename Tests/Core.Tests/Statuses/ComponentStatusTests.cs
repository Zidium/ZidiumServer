...
namespace Zidium.Core.Tests.Statuses
{
    public class ComponentStatusTests : BaseTest
    {
        ...
        public void ParentComponentTest()
        {
            ...
            Assert.True(statusResponse.Success);
            Assert.Equal(MonitoringStatus.Success, statusResponse.GetDataAndCheck().Status);
            ...
            statusResponse = childComponent.GetTotalState(false);
            Assert.True(statusResponse.Success);
            Assert.Equal(MonitoringStatus.Alarm, statusResponse.GetDataAndCheck().Status);
        ...
        public void UnitTestResultTest()
        {
            ...
            Assert.True(statusResponse.Success);
            Assert.Equal(MonitoringStatus.Warning, statusResponse.GetDataAndCheck().Status);
            Assert.True(statusResponse.GetDataAndCheck().ActualDate.Year > DateTime.Now.Year);
        ...
        public void NonActualStatusTest()
        {
            ...
            Assert.True(sendResponse.Success);
            var statusResponse = component.GetTotalState(false);
            Assert.Equal(MonitoringStatus.Warning, statusResponse.GetDataAndCheck().Status); // статус должен быть Warning
            ...
            var uniTestState = unitTest.GetState().GetDataAndCheck();
            Assert.Equal(MonitoringStatus.Alarm, uniTestState.Status);
            Assert.False(uniTestState.HasSignal);
            ...
            MonitoringStatus externalStatus)
        {
            ...
            var internalState = client.ApiService.GetComponentInternalState(control.Info.Id, false).GetDataAndCheck();
            Assert.Equal(internalStatus, internalState.Status);
            var totalState = client.ApiService.GetComponentTotalState(control.Info.Id, false).GetDataAndCheck();
        ...
        public void ComplexTest()
        {
            ...
            Assert.Equal(MonitoringStatus.Alarm, unitTestState.GetDataAndCheck().Status);
            //account.SaveAllCaches();
        ...
        private List<DbEvent> GetEvents(TestAccountInfo account, Guid componentId, Guid ownerId, bool ignoreCheck = false)
        {
            ...
            using (var accountDbContext = TestHelper.GetDbContext())
            {
                ...
                using (var storageDbContext = account.GetDbContext())
                {
                    ...
                    foreach (var eventObj in events)
                    {
                        ...
                        if (eventObj.StartDate == component.CreatedDate ||
                            eventObj.Importance == EventImportance.Unknown)
                        {
                            continue;
                        }
        ...
        private void Check4Events(List<DbEvent> events)
        {
            ...
            Assert.Equal(EventImportance.Unknown, events[0].Importance);
            ...
            Assert.Equal(EventImportance.Success, events[1].Importance);
            ...
            Assert.Equal(EventImportance.Alarm, events[2].Importance);
            Assert.Equal(events[2].EndDate, events[3].StartDate);

            //Assert.Equal(events[2].Duration, TimeSpan.Zero);
            Assert.Equal(EventImportance.Success, events[3].Importance);
        }
        ...
        public void AppErrorStatusEvents()
        {
            ...
            Assert.Equal(MonitoringStatus.Success, state.GetDataAndCheck().Status);
            ...
            TestHelper.WaitTrue(() =>
            {
                ...
                return state.GetDataAndCheck().Status == MonitoringStatus.Unknown;
            }, TimeSpan.FromMinutes(1));

            var events = GetEvents(account, component.Info.Id, component.Info.Id);
            var externalStatuses = events
                .Where(x => x.Category == EventCategory.ComponentExternalStatus)
        ...
        public void UnitTestsStatusEvents()
        {
            ...
            using (var accountDbContext = account.GetDbContext())
            {
                ...
                var unitTestResults = unitTestEvents
                    .Where(x => x.Category == EventCategory.UnitTestStatus)
                    .OrderBy(x => x.CreateDate)
                    .ToList();
                ...
                var unitTestsStatuses = componentEvents
                    .Where(x => x.Category == EventCategory.ComponentUnitTestsStatus)
                    .OrderBy(x => x.CreateDate)
                    .ToList();
                ...
                var internalStatuses = componentEvents
                    .Where(x => x.Category == EventCategory.ComponentInternalStatus)
                    .OrderBy(x => x.CreateDate)
                    .ToList();
        ...
        public void OfflineComponent1()
        {
            ...
            state = component.GetTotalState(false);
            Assert.Equal(MonitoringStatus.Disabled, state.GetDataAndCheck().Status);
            ...
            var successEvents = events4
                .Where(x => x.Category == EventCategory.ComponentExternalStatus
                            || x.Category == EventCategory.ComponentInternalStatus
            ...
            var unknownEvents = events4
                .Where(x => x.Category == EventCategory.ComponentUnitTestsStatus
                            || x.Category == EventCategory.ComponentMetricsStatus
                            || x.Category == EventCategory.ComponentChildsStatus)
                .ToList();
        ...
        public void OfflineComponentTest()
        {
            ...
            var state = component.GetTotalState(false);
            state.Check();
            Assert.Equal(MonitoringStatus.Unknown, state.GetDataAndCheck().Status);
