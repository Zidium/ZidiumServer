...
namespace Zidium.Core.Tests.Dispatcher
{
    ...
    public class TsdFixEventsTests : BaseTest
    {
        ...
        public void ProcessSimpleEventThreadSafe2()
        {
            ...
            var queue = new Zidium.Api.ThreadTaskQueue(threadCount);
            var responses = new List<SendEventResponseDto>();
            ...
            var eventIds = responses.Select(x => x.GetDataAndCheck().EventId).Distinct().ToList();
        ...
        private void UpdateTsdStatus(object componentObj)
        {
            ...
            var response = dispatcherClient.UpdateComponentState(componentId);
            if (response.Success == false)
            {
                response.Check();
            }
