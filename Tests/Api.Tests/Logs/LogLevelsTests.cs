...
namespace Zidium.Api.Tests.Logs
{
    public class LogLevelsTests : BaseTest
    {
        ...
        public void Test()
        {
            ...
            {
                Assert.DoesNotContain(level + ":text 0", messages);
                Assert.Contains(level + ":text 1", messages);
                Assert.Contains(level + ":text 2; Name=Alexey; Login=Smirmov", messages);
                Assert.Contains(level + ":text 3; Id=100200; DisplayName=Test display name", messages);
                Assert.Contains(level + ":text 4", messages);
                Assert.Contains(level + ":text 5; Name=Alexey; Login=Smirmov", messages);
            }
            ...
            {
                Assert.DoesNotContain(level + ":text 0", messages);
                Assert.Contains(messages, t => t.StartsWith(level + ":Mega exception;"));
                Assert.Contains(messages, t => t.StartsWith(level + ":test 1 : Mega exception;"));
                Assert.Contains(messages, t => t.StartsWith(level + ":test 2 : Mega exception; Name=Alexey; Login=Smirmov;"));
                Assert.Contains(messages, t => t.StartsWith(level + ":test 3 : Mega exception; Id=100200; DisplayName=Test display name;"));
            }
        }
    }
}
