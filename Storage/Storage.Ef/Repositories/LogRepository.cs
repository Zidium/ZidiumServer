...
namespace Zidium.Storage.Ef
{
    internal class LogRepository : ILogRepository
    {
        ...
        public void Add(LogForAdd[] entities)
        {
            using (var contextWrapper = _storage.GetContextWrapper())
            {
                contextWrapper.Context.Configuration.AutoDetectChangesEnabled = false;
                try
                {
                ...
                {
                    contextWrapper.Context.Configuration.AutoDetectChangesEnabled = true;
                }
                contextWrapper.Context.SaveChanges();
            }
        }
        ...
        public int DeleteLogProperties(int maxCount, DateTime toDate)
        {
            using (var contextWrapper = _storage.GetContextWrapper())
            {
                ...
                var objectQuery = (ObjectQuery)((IObjectContextAdapter)contextWrapper.Context).ObjectContext
                    .CreateObjectSet<DbLogProperty>()
                ...
                using (var connection = contextWrapper.Context.CreateConnection())
                {
                    ...
                    using (var command = connection.CreateCommand())
                    {
                        ...
                        var query = $"DELETE FROM {contextWrapper.Context.FormatTableName("LogParameters")} WHERE {contextWrapper.Context.FormatColumnName("Id")} IN ({objectQuery.ToTraceString()})";
        ...
        public int DeleteLogs(int maxCount, DateTime toDate)
        {
            using (var contextWrapper = _storage.GetContextWrapper())
            {
                var objectQuery = (ObjectQuery)((IObjectContextAdapter)contextWrapper.Context).ObjectContext.CreateObjectSet<DbLog>()
                    .Where(t => t.Date < toDate).OrderBy(t => t.Date).Select(t => t.Id).Take(maxCount);
                ...
                using (var connection = contextWrapper.Context.CreateConnection())
                {
                    ...
                    using (var command = connection.CreateCommand())
                    {
                        ...
                        var query = $"DELETE FROM {contextWrapper.Context.FormatTableName("Logs")} WHERE {contextWrapper.Context.FormatColumnName("Id")} IN ({objectQuery.ToTraceString()})";
